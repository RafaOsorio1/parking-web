...
import { useSpots, useCreateSpot } from "../hooks/useParking";
import { Modal } from "../components/Modal";
import type { Rate } from "../types/parking";

export function Settings() {
  const { rates, isLoading: isLoadingRates, updateRate, createRate } = useRates();
  const {
    data: spotsResponse,
  ...
  const handleGenerateSpots = async () => {
    ...
    const existingSpots = spots.filter((s) => s.type === spotConfig.type).length;
    let successCount = 0;
    ...
    <div className="space-y-12 pb-12">
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl md:col-span-1 space-y-4 shadow-xl">
            <h3 className="text-lg font-semibold text-slate-300">Generar Lote</h3>
            <div>
            ...
            <div>
              <label className="text-sm text-slate-400 block mb-2">Cantidad</label>
              <input
                type="number"
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-blue-500 outline-none"
          ...
          <div className="md:col-span-2 grid grid-cols-2 gap-6">
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex flex-col items-center justify-center shadow-xl">
              ...
              <p className="text-slate-400 mt-2 font-medium">Espacios de Carro</p>
            </div>
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex flex-col items-center justify-center shadow-xl">
              ...
              <p className="text-slate-400 mt-2 font-medium">Espacios de Moto</p>
            </div>
          </div>
        </div>
      </section>
                ...
                {editingRate?.id === rate.id ? (
                  <div className="space-y-4 animate-in fade-in zoom-in-95">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs text-slate-500 uppercase font-bold">Por Hora</label>
                        <input
                          type="number"
                          ...
                          onChange={(e) =>
                            setEditingRate({ ...editingRate, hourlyRate: parseFloat(e.target.value) || 0 })
                          }
                        />
                      </div>
                      <div>
                        <label className="text-xs text-slate-500 uppercase font-bold">Base (Fracción)</label>
                        <input
                          type="number"
                          className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-white outline-none"
                          value={editingRate.baseFee}
                          onChange={(e) =>
                            setEditingRate({ ...editingRate, baseFee: parseFloat(e.target.value) || 0 })
                          }
                        />
                      </div>
                    </div>
                  ...
                  <>
                    <h2 className="text-2xl font-bold mb-2 text-white">{rate.name}</h2>
                    <div className="space-y-4 mt-6">
                      ...
                      <div className="flex items-center justify-between text-slate-300">
                        ...
                        <span className="text-white font-medium">
                          {rate.baseTimeMinutes} min / ${rate.baseFee.toLocaleString()}
                        </span>
                      </div>
                    </div>
      ...
      >
        <div className="space-y-4">
          <div>
            <label className="text-xs text-slate-500 uppercase font-bold ml-1">Nombre</label>
            <input
              ...
              onChange={(e) => setNewRateData({ ...newRateData, name: e.target.value })}
            />
          </div>
          <div>
            <label className="text-xs text-slate-500 uppercase font-bold ml-1">Tipo de Vehículo</label>
            <select
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white outline-none"
              value={newRateData.vehicleType}
              onChange={(e) => setNewRateData({ ...newRateData, vehicleType: e.target.value as any })}
            >
              <option value="CAR">Carro</option>
              <option value="MOTORCYCLE">Moto</option>
            </select>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-slate-500 uppercase font-bold ml-1">Tarifa Hora</label>
              <input
                type="number"
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white outline-none"
                value={newRateData.hourlyRate}
                onChange={(e) => setNewRateData({ ...newRateData, hourlyRate: parseFloat(e.target.value) || 0 })}
              />
            </div>
            <div>
              <label className="text-xs text-slate-500 uppercase font-bold ml-1">Base Fracción</label>
              <input
                type="number"
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white outline-none"
                value={newRateData.baseFee}
                onChange={(e) => setNewRateData({ ...newRateData, baseFee: parseFloat(e.target.value) || 0 })}
              />
            </div>
          </div>
        </div>
