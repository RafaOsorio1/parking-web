    ...
    <div className="space-y-12 max-w-5xl mx-auto pb-12">
      ...
      {!session ? (
        ...
        <div className="bg-slate-900 border border-slate-800 p-10 rounded-3xl shadow-2xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="max-w-md mx-auto space-y-6">
            ...
            <button
              onClick={handleOpen}
              disabled={openMutation.isPending || !initialAmount || !operatorName}
          ...
          <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl shadow-xl h-fit sticky top-8 space-y-6">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <CircleDollarSign className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={20} />
                  <input
                    type="number"
                    value={actualAmount}
      ...
      <section className="space-y-6">
        ...
        {history.length === 0 ? (
          <div className="bg-slate-900/50 border border-dashed border-slate-800 rounded-3xl p-12 text-center">
            <p className="text-slate-500">No hay cierres registrados todavía.</p>
          </div>
        ) : (
            ...
            {history.map((item) => (
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    <p className="text-xs font-bold text-slate-500 uppercase">Cerrado por</p>
                ...
                <div className="space-y-2">
                   <p className="text-xs text-slate-500 font-medium">
                      {new Date(item.openingTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} - 
                      {new Date(item.closingTime!).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                   </p>
                   <div className="flex justify-between items-center">
                    <span className="text-sm text-slate-400">Ventas</span>
                    <span className="font-bold text-white">${item.totalIncome.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-slate-400">Diferencia</span>
                    <span className={`flex items-center gap-1 font-bold ${
                      (item.difference || 0) >= 0 ? "text-emerald-500" : "text-red-500"
                    }`}>
                ...
                <div className="pt-4 border-t border-slate-800/50 flex items-center justify-between">
                   <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest">Responsable: {item.openedByName}</span>
                   <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest">{item.id.slice(0,8)}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      >
        <div className="space-y-6">
          <div className="space-y-3">
             <label className="text-sm font-bold text-slate-400 uppercase tracking-widest ml-1">Nombre del Responsable</label>
             <input
               type="text"
               value={operatorName}
               onChange={(e) => setOperatorName(e.target.value)}
          ...
          <div className="p-5 bg-slate-950 rounded-2xl border border-slate-800 space-y-3">
            <div className="flex justify-between items-center text-slate-400">
              <span>Esperado:</span>
              <span className="text-white font-bold">${session?.currentExpected?.toLocaleString()}</span>
            </div>
            <div className="flex justify-between items-center text-slate-400">
              <span>Contado:</span>
              <span className="text-white font-bold">${parseFloat(actualAmount).toLocaleString()}</span>
            </div>
            <div className="pt-3 border-t border-slate-800 flex justify-between items-center">
              <span className="font-bold">Diferencia:</span>
              <span className={`text-xl font-black ${
                parseFloat(actualAmount) - (session?.currentExpected || 0) >= 0 ? "text-emerald-500" : "text-red-500"
              }`}>
                ${(parseFloat(actualAmount) - (session?.currentExpected || 0)).toLocaleString()}
              </span>
            </div>
          </div>
        </div>
