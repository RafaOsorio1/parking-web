...
export function DashboardStats({ activeTickets, spots, apiStatus }: DashboardStatsProps) {
  const totalSpots = spots.length;
  const occupiedSpots = spots.filter((s) => s.status === "OCCUPIED").length;
  const occupancyPercent = totalSpots === 0 ? 0 : Math.round((occupiedSpots / totalSpots) * 100);

  const carTickets = activeTickets.filter((t) => t.vehicle.type === "CAR").length;
  const bikeTickets = activeTickets.filter((t) => t.vehicle.type === "MOTORCYCLE").length;
