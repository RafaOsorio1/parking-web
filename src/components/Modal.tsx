  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
      ...
      <div className='bg-slate-900 border border-slate-800 w-full max-w-md rounded-3xl shadow-2xl relative z-10 overflow-hidden animate-in zoom-in-95 duration-200'>
        <div
          className={`p-8 text-center border-b border-slate-800 ${variantStyles[variant]}`}
        >
          <div
            className={`${iconBgStyles[variant]} w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg`}
          >
        ...
        <div className='p-8 space-y-6'>
          ...
          {footer && (
            <div className='pt-4 border-t border-slate-800'>{footer}</div>
          )}
        </div>
